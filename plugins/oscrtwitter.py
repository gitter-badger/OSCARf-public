...
def mentionCount(t_api):
  ...
  if opt == "1":
    ...
    user = t_api.get_user(targetUsr)
    target_tweets = t_api.user_timeline(targetUsr,count=200) #200 is the api limit per query currently?
  ...
  if opt == "2":
    try:
      ...
      print "There was an error opening the tweets file. Did you run the first option?\n"
      return
    ...
    while True:
      try:
        targetUsr2 = raw_input("Enter a twitter handle (without the @) Ctrl+C to return: ")
        print "Number of times mentioned: ",names.count(targetUsr2)
      except(KeyboardInterrupt):
        print "\n"
        return
  #twitMenu()
...
def twitSearch(t_api):
  ...
  t_res = tweepy.Cursor(t_api.search,q=t_query,count=10,result="recent",include_entities=True).items()
  while True:
    try:
      tweet = t_res.next()
      print tweet.user.screen_name.encode("utf-8"),":",tweet.created_at,":",tweet.text.encode("utf-8")
...
def twitlookup(t_api):
  ...
  if data == 0:
    ...
    try:
      ...
      VALUES (?,?,?)",(targetUsr,followers,lastTweet))
      conn.commit()
    except:
    ...
    print "Latest tweet: \n",lastTweet
    try:
      c.execute("UPDATE twitter set followers = ?, lasttweet = ? where username = ?",(followers,lastTweet,targetUsr))
      conn.commit()
      conn.close()
    except:
      print "[+]ERROR: Could not update databse"
